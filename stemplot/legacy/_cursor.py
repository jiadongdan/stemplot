...
class Cursor(AxesWidget):
    ...
    def onclick(self, event):
        ...
        letter = "fig_add_letter(fig, x={:.4f}, y={:.4f}, letter='a',)".format(x, y)
        print(ss)
        print(cc)
        print(letter)
...
class FigCursor(AxesWidget):

    def __init__(self, fig, horizOn=True, vertOn=True, useblit=True, transform=None,
                 **lineprops):
        """
        Add a cursor to *ax*.  If ``useblit=True``, use the backend-dependent
        blitting features for faster updates.  *lineprops* is a dictionary of
    ...
    def onclick(self, event):
        ...
        ss = "ax.text(x={:.4f}, y={:.4f}, s='text', ha='left', va='top', transform=ax.transAxes)".format(x, y)
        cc = "c = plt.Circle(({:.4f}, {:.4f}), radius=0.02, fill=False, transform=ax.transAxes)".format(x, y)
        letter = "fig_add_letter(fig, x={:.4f}, y={:.4f}, s='a',)".format(x, y)
        ax_ss = 'ax = make_ax(fig, x={:.4f}, y={:.4f}, w=0.1, hrel=1)'.format(x, y)
        print(ss)
        print(cc)
        print(letter)
        print(ax_ss)
